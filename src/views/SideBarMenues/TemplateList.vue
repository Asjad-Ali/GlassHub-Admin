        ...
        <div class="card-body pb-0">
            <div class="row mb-5">
                ...
                <div class="col-md-6 text-right">
                    <router-link to="/template/add-template"><button class="btn btn-primary">Add</button></router-link>
                </div>
            </div>
            <div class="row mb-2">
                <div class="col-md-6">
            ...
            <div class="row">
                ...
                <div v-else>
                    <div v-if="store.allTemplateList.length" >
                        <div  class="row">
                                ...
                                <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                    <thead>
                                        <tr class="fw-bolder text-muted">
                                            ...
                                            <th class="min-w-140px">Template Name </th>
                                    ...
                                    <tbody>
                                        <tr v-for="(page,index) in store.allTemplateList" :key="page">
                                            <td><span class="text-muted fw-bold text-muted d-block fs-7">{{ index+1 }}</span></td>
                                            <td><span class="text-muted fw-bold text-muted d-block fs-7"></span>{{page.created_at.substring(0, 10)}}</td>
                                            <td><span class="text-muted fw-bold text-muted d-block fs-7"></span>{{page.Account_name}}</td>
                                            <td><span class="text-muted fw-bold text-muted d-block fs-7"></span>{{page.Template_type_name}}</td>
                                            <td>
                                            ...
                                            <td class="text-end">
                                                <ul class="userslist_action_icons">
                                                    ...
                                                    <li><i @click="editTemplate(page)" class="fa fa-pencil-square cursor-pointer" aria-hidden="true"></i></li>
                                                    <li @click="ShowModel()"><i @click="id = page.Template_id " class="fa fa-trash cursor-pointer" aria-hidden="true"></i></li>
                                                    <!-- @click="id = issue.H_id " -->
                                                </ul>
                                            </td>
                                        </tr>
                        ...
                        <div class="col-md-12">
                            <div style="display:flex ; justify-content:center; margin-top:10px">
                                ...
                                :range-size="1"
                                active-color="#DCEDFF"
                                @update:modelValue="loadtemplateList"
                                />
                            </div>
                        </div>
                    </div>
...
const deleteRecord = () => {
    store.deleteTemplateList(id.value)
}
...
const loadtemplateList = () => {
    store.loadTemplateList(data.value)
}
...
const ShowModel = () => {
    ...
    console.log(page)
    localStorage.setItem('editTemplate',JSON.stringify(page))
    router.push({name: "EditTemplate", params: { id: page.Template_id }})
}
